...
Сценарий: Подготовка данных для проверки
	...
	И я проверяю или создаю для документа "ОперацияПоУчетуТоваров" объекты:
		...
		| 'e1cib/data/Документ.ОперацияПоУчетуТоваров?ref=8ca7000d8843cd1b11dc99ca10d37aab' | 'False'           | '000000001' | '05.09.2020 1:11:26' | 'False'    | 'Корректировка количества товара на складе по результатам инвентаризации' | 'e1cib/data/Справочник.Организации?ref=8d34000d8843cd1b11dd2bea12f94c65' |
	...
	И я проверяю или создаю для регистра накоплений "ТоварныеЗапасы" записи:
		...
		| '05.09.2020 1:11:26'  | 'e1cib/data/Документ.ОперацияПоУчетуТоваров?ref=8ca7000d8843cd1b11dc99ca10d37aab' | 1             | 'True'       | 'Приход'      | 'e1cib/data/Справочник.Товары?ref=8ca1000d8843cd1b11dc8eb360f70b4d' | 'e1cib/data/Справочник.Склады?ref=a9b000055d49b45e11db8c4c9d5c4221' | 4            |

Сценарий: Подготовка к запуску сеанса продавца
	// Ставим расширение с помощью своего шага
	Если расширение с именем "Расширение1" установлено Тогда
		// Ничего не делаем
	Иначе
		И я устанавливаю в данную базу расширение из файла "$КаталогПроекта$\Расширение1.cfe"
...
Сценарий: Проведение документа
	...
	* Заполняем поля и товары без подбора
